...
const DiseaseDetectionForm: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    ...
    const hasSelectedSymptoms = Object.values(values).some(value => value === true);

    if (!hasSelectedSymptoms) {
      setLoading(false);
      // Set a special "healthy" result
      setResult({
        ...
        active_symptoms: [],
        recommendations: {
          general_advice: "Your cattle appears to be healthy as no symptoms were reported. Continue with regular health monitoring and preventive care.",
          prevention: "Maintain good hygiene, proper nutrition, and regular health checks to keep your cattle healthy."
        },
        model_info: {
          type: "Health Check",
          path: "",
          accuracy: 1,
          macro_f1: 1,
          feature_count: Object.keys(SYMPTOMS).length
        }
      });
      return;
    }
              ...
              <div className="grid gap-6">
                ...
                >
                  <h3 className="text-xl font-semibold mb-2">
                    {result.predicted_disease === 'Healthy' ? 'Health Status' : 'Primary Diagnosis'}
                  </h3>
